# COMPLETED — GitHub Issues #38–#52

**This file is the archive. Open work lives in [todo.md](todo.md).**

Split out of `todo.md` on 2026-07-30 once the code side of the #38–#52 batch was
done. Everything here shipped on `dev`, with its commit hash. Each section keeps
its **correction and trap blocks** — that is the reason this file exists rather
than a `git log` one-liner per issue. **Fourteen of the plans in here were wrong
before they were right**, and the corrections next to them are the only record of
why. Read the relevant section before re-planning anything that touches the same
code.

Where a section still has an open remainder (a deploy, a device check, a
follow-up), the item itself is **not** repeated here — there is one line pointing
at `todo.md`, so no checkbox has two homes.
...
- [x] Follow-up defect: `blobs`/`trash`/`photos` used **timestamp-only** cursors (`upload_time` / `deleted_at` / `taken_at`) with a strict boundary, so a page cut inside a same-timestamp run dropped rows. Not theoretical: batch-encrypted blobs, an emptied trash and a batch import each share a timestamp in bulk. Composite cursor (`ts|id`; photos `ts|filename|id`), pool-only `list_*_page` helpers, RED-verified. — `8d55a64`

> **Open remainder → [todo.md](todo.md):** deploy CT132 to realise the fix (the
> 29 lost photos and the badge numbers do not change until the box runs it).
...
- [x] **Android `?since=` adoption — ALREADY SHIPPED; this box was stale.** Checked 2026-07-22: `PhotoRepository.syncFromServer` already has `decideSyncMode` (skip / delta / full), the head-skip fast path, `applyTombstones` kept strictly apart from `reconcileServerDeletions` (delta ids must never reach the reconciler, or a delta page prunes everything it did not mention), first-page-head persistence, and a **Room-backed** `SyncCursorStore` — cleared with the mirror, and refusing a cursor over an empty mirror, which is the co-location rule the web hazard list demanded. `SyncPlanTest` (13). **Do not re-implement it**; verify before believing an unchecked box in this file.

> **Open remainder → [todo.md](todo.md):** tombstone retention policy; deploy
> (`033` is the first migration here carrying a data backfill).
...
> **Open remainder → [todo.md](todo.md):** the #51 **server** half (still
> HYPOTHESIS — with both clients shown virtualized and bounded, a thumbnail
> request storm is what is left to measure), and observing the web fix against a
> real large library rather than a fixture.
...
> **Open remainder → [todo.md](todo.md):** persist the measured per-category
> rates across passes (today *every* server boot is a machine that has never
> converted anything), and observe a real mixed pass to see whether the seeded
> video rate is within ~2× of CT132's hardware.
...
> **Open remainder → [todo.md](todo.md):** the E2E that drives five real scan
> passes, and the deploy (the Takeout duplicate-transcode loop is the one worth
> measuring before/after — it is pure wasted CPU today).
...
> **Open remainder → [todo.md](todo.md):** `needs_web_preview` still has the
> extension-only blind spot (deliberately deferred from `6663a4c`), and the
> deploy-state re-verification below.
...
**DEPLOY-STATE TRAP (2026-07-22) — keep this lesson.** The #46 backfill was
**never actually deployed** when we set out to "test backfilling un-wiped". CT132
was found running commit `214ced8` (image built 21:51 UTC 2026-07-21); `6663a4c`
(#46 backfill) landed ~3h later, and `d1f439a` (#43) too -- the box was 4 commits
behind dev. The symptom that caught it: the live `[LADDER]` log still used the
OLD `candidates=N` format with no `backfill_examined`/`backfill_produced` fields
that `6663a4c` added. **Confirm the deployed commit (box `git HEAD` + log schema)
before trusting a "we deployed it" -- container uptime proves nothing about which
code is in it.**

> **Open remainder → [todo.md](todo.md):** redeploy and re-verify that the
> backfill actually drains. **B3a (parked/unencrypted photos), discovered here,
> is entirely open and lives in `todo.md` in full.**
...
> is slow.
- [x] **Orphan sweeper.** `035`'s cascade queued unreferenced rendition blobs into `orphaned_rendition_blobs` and **nothing drained it**, so a deleted 4K video leaked its rendition bytes forever. New `transcode::orphan_sweep`, run **first** in every `run_sweep`; it re-checks references (renditions / photos / `encrypted_gallery_items`) before unlinking, because `blobs.content_hash` dedup means one blob can back several photos, and is bounded at 200/pass and concurrency-safe. `037` had already removed the worst trap — source rungs never reach this queue, so it can only ever name bytes a rendition owns. — `b4548f9`

> **Open remainder → [todo.md](todo.md):** cost control — the sweep is
> **serial**, so a 114-file backlog of mostly-4K sources drains slowly. Decide
> whether it gets a lane of the two-lane parallelism or stays serial by choice.
...
- [x] **`rung_queue::geometry_is_known` was dead** (the last `dead_code` warning) and its doc claimed a caller — "the generation pass branches on this" — that `rung_generate` did not have. Now feeds a `blind` count in the sweep log, so "narrowed by the prefilter" is distinguishable from "selected blind" instead of a comment asserting a caller that does not exist. — `037c041`

> **Open remainder → [todo.md](todo.md):** decide whether the secure gallery's
> own item listing should carry the ladder — securing a video removes its picker.
...
> **Open remainder → [todo.md](todo.md):** web device-verify of the quality swap
> (playhead restore, pause-state restore, no flash of the original) — it has
> never run in a browser; this repo has no jsdom.
...
> **Open remainder → [todo.md](todo.md):** device-verify the quality swap on
> both clients, drain the 136-file backfill on the live box, and the one test
> shape not yet written — rendition serving + range requests in E2E. (Ladder
> selection and the picker default per network state *are* unit-tested.)
...
**(b) — DONE**, commit `9046915`. **The A1 chain in the hypothesis above was not the mechanism.**

- [x] The real cause is narrower and worse: `fetch_face_clusters` picked the highest-confidence detection **unconditionally**, with no eligibility test on the photo carrying it. A cluster whose best face sits on a **secured** photo therefore resolved to a photo no client mirror holds — placeholder tile — and, had it resolved, would have **cropped and displayed a face out of a hidden photo**. So the "missing thumbnail" symptom and a secure-gallery leak were the same line of SQL. — `9046915`
- [x] The representative now joins `photos` and picks the highest-confidence detection **whose photo is eligible**, which is exactly the "fall back to the next resolvable detection" the plan asked for — implemented in the query rather than as a client retry. — `9046915`
- [x] The handler logs when a cluster resolves no thumbnail at all, so the genuine remainder is visible instead of silently placeholdered. — `9046915`
...
> **Open remainder → [todo.md](todo.md):** **pets cannot be framed here — that
> plan was wrong.** `PetCluster` carries no `rep_bbox_*` on the server, in
> `web/src/api/ai.ts`, or in Android's `AiDto.kt`; only `FaceCluster` does. It is
> a server-side task, not a client wire-up.
...
> **Open remainder → [todo.md](todo.md):** verify on a device with 3-button nav
> AND gesture nav (different inset heights), and the iOS/PWA half on a notched
> device — every `env()` value is 0 on the dev machine, so the desktop-identical
> rendering proves the no-op property and nothing else.
...
> **Open remainder → [todo.md](todo.md):** device-verify the
> selection→dialog→move gesture on the S21+ harness.
...
**DONE** — web `a008500`, Android `b339a32`. 304 web green (was 275); full Android
unit suite green.

> **The sort had to be OPTIONAL, and that is the whole design.** `null` means
> "no choice made" and keeps the album's **intrinsic** order — which is what
> preserves Recently Added's add-order, and why every pre-existing test stayed
> green without being touched (they pass no sort). Only an explicit pick
> post-sorts. A mandatory default would have silently rewritten the ordering of
> every album in the product to prove a feature works.
>
> **The smart-album limit is applied BEFORE the sort.** Recently Added still
> selects the N most-recently-*added*; the user's choice reorders that set. Sort
> first and the cap starts choosing *which* photos are in the album, so picking
> "Name ascending" would change the membership of Recently Added.
>
> **Sorted after burst collapse, by the representative frame** — the recurring
> burst/order trap named in this file's cross-cutting risks. Sort the raw rows
> and a burst tile jumps to wherever its non-cover frames landed.
>
> **A deterministic id tiebreak is load-bearing, not tidiness.** Equal keys that
> swap between renders produce a new array identity every pass, and
> `JustifiedGrid` needs order stability to hold scroll position (#51).

- [x] Sort control on the right of the album-detail header: Date and Name, each ascending/descending. Active field reverses, the other field switches. — `a008500` / `b339a32`
- [x] Implemented once in `resolveAlbumPhotos` (web) / `AlbumDetailViewModel.applySort` (Android) so every album kind — regular and all six smart albums — inherits it, never special-cased per view. — `a008500` / `b339a32`
- [x] Persisted per album: `albumSort:<id>` in localStorage (web) and the Preferences DataStore (Android), read before the first render; a corrupt or absent value reads as "no choice". The `useScrollMemory` precedent. — `a008500` / `b339a32`
- [x] Burst collapse: sorted **after** collapsing, by the representative frame. — `a008500`
- [x] Locale-aware name sort — `Intl.Collator(numeric: true, sensitivity: base)` on web, a numeric-aware case-insensitive natural compare on Android, so `IMG_2` precedes `IMG_10`. — `a008500` / `b339a32`
- [x] Android parity, with the decision logic ported pure to `data/album/AlbumSort.kt` (key selectors, no Android/Room types — the `RenditionChoice.kt` split). The ViewModel holds the intrinsic-order base list and re-derives on a sort change **without a reload**. — `b339a32`
- [x] Comparators unit-tested including ties and missing `takenAt`: 16 web (`albumSort.test.ts`) + 4 `resolveAlbumPhotos` integration tests + 14 Android (`AlbumSortTest`). — `a008500` / `b339a32`

> **Open remainder → [todo.md](todo.md):** **E3 — the photo viewer did not
> respect this sort.** Reported 2026-07-30, confirmed on Android. Fixed below.

---

### E3 — #52 follow-up: the photo viewer ignored the album sort (Android) — CONFIRMED

**Reported:** with the album sorted by date the grid renders correctly, but
opening a photo and paging through the viewer shows a completely different order.

**Root cause: the grid and the pager were two derivations of one query, and only
one applied the user's choices.** `AlbumDetailViewModel` ran `getAlbumPhotos →
excludeSecure → sortAlbumItems`; `PhotoViewerViewModel` ran `getAlbumPhotos →
collapseBursts` and nothing else. The tapped photo is located by id, which is
...
> would have caused a second bug.** E3 read divergence 3 as *"the pager collapses
> bursts, the grid does not, so make the grid collapse."* It is the other way
> round. `web/src/hooks/useAlbumPhotos.ts:60-62` collapses for **smart albums
> only**, deliberately: *"regular albums keep every frame a user explicitly
> added, so removal/secure-add over the rendered list stays faithful to the
> manifest."* `getAlbumPhotos` already encoded exactly that, so the **pager's**
> unconditional collapse was the wrong side. Collapsing the regular-album grid
> would have made Android diverge from web *and* broken the manifest-faithfulness
> album removal depends on. **Check the web resolver before deciding which side
> of an Android/web divergence is correct** — the older code is not automatically
> the wrong one.
>
> **The burst policy could not move into the new resolver**, which is why it
> stayed in `getAlbumPhotos`: `smart-recents` must collapse **before** it caps to
> 100, or a 46-shot burst eats 46 of the 100 slots. Collapse-then-cap is not
> expressible as a post-filter.
>
> **`-1` from `pageIndexOf` is a real answer, and finding that was the point of
> doing the audit.** Excluding secured photos from the pager means a tap can now
> name an id the list does not hold — search and the people/pets/memories/trips
> grids all resolve from **server** endpoints, so they always could. The old code
> coerced that to page 0, silently opening an unrelated photo and calling it the
> one the user tapped; for a secured id that is the same leak wearing a different
> hat. It now renders "Photo not found" and logs.
>
> **Two derivations of one list will drift — fifth instance.** #42 (three
> definitions of "count"), #48(a) (two copies of one crop formula), #51 (two
> owners of one blob URL), E3 (two derivations of one album list), and the
> viewer's own `setRawPhotos`, which re-derived `allPhotos` from `allPhotosRaw`
> on every favourite/crop/subtype edit. The fix is always one function called
> twice.

- [x] **Android: one resolver, called once.** New `data/album/AlbumPhotoResolver.kt`:
      a pure `resolvePhotos(members, secureBlobIds, sort, collapseBurstStacks)`
      plus a thin injected fetch-and-delegate class. **Neither ViewModel calls
      `getAlbumPhotos` any more**, so they have no way left to build a divergent
      list — equal by construction, not by convention.
- [x] Order of operations pinned as `excludeSecure → collapse → sort`. Excluding
      first means a secured frame can never become a burst's representative;
      collapsing before sorting means changing the sort never swaps a stack's
      cover image.
- [x] **Defect 1 — the pager did not exclude secure photos.** Fixed for the album
      path *and* the gallery path, which had no filter at all
      (`getAllPhotos()` raw). Treated as the confidentiality bug it is.
- [x] **Defect 2 — burst policy.** Resolved in favour of web's (see correction
      above): the pager stopped collapsing unconditionally.
- [x] The `loadPhotos` comment that asserted the opposite of what the code did is
      gone. D1/#44, B2/#40 and E3 all shipped with one; it is what stopped this
      being caught.
- [x] `GalleryScreen`'s inline re-implementation of `collapseBursts` replaced with
      the shared helper — it was a fourth copy, in the surface the resolver's
      gallery branch has to match exactly.
- [x] `setRawPhotos` replaced by `updateLoadedPhotos`/`removeLoadedPhoto`, which
      edit both lists in step instead of re-deriving one from the other.
- [x] **Test (JVM, no device):** `AlbumPhotoResolverTest`, 15 cases. Asserts
      **equality of the two lists**, not "the pager is sorted" — a pager that
      sorts correctly but resolves a different membership still strands the user.
      `pagerListBeforeTheFix()` reproduces the old derivation verbatim so every
      case is asserted against the real defect, and fails if anyone reintroduces
      a second derivation.
- [x] **Audit of every other Android viewer entry point.** Gallery, search,
...
> **Open remainder → [todo.md](todo.md):** device-verify that two real windows
> are counted as two and that killing one re-enables the menu entry.
...
## Execution order, as it actually ran (historical)

The order below is the plan; every line has since been closed. Kept because the
**dependencies** it names are the reusable part — A1 gated the honest measurement
of C1(b), B1 gated B2/B3, and B2's cap gated B3's backfill.

1. ~~**A1** (#42)~~ **DONE** â€” counts + the pagination off-by-one (`568c282`, `29e4d1f`, `21eae82`), plus the same-timestamp cursor follow-up (`8d55a64`). Everything downstream is measured against correct numbers.
2. ~~**A3** (#51)~~ **web DONE** `20690ad`; Android's two preconditions already held. Only the server-side hypothesis is left.
3. ~~**A2** (#38)~~ **DONE** â€” client batching `f31b27b`, server delta `31fc322`, web `?since=` `6a1b711`, Android already shipped.
...
6. ~~**C1** (#48) — face centering.~~ **(a), (c), (d-People) DONE** 2026-07-21 — `5c4d776`; **(b) DONE** 2026-07-22 — `9046915` (and it was *not* the A1 chain the plan predicted — it was an unconditional highest-confidence pick that could resolve to a secured photo). Only (d-Pets) is left, and it is a server task. Read the (a) corrections before trusting any other plan in this file: the formula was right, the *mechanism* was not, and the existing test suite asserted the bug.
7. ~~**B3** (#46) — codec probing.~~ **Code `65389a7`; backfill + corrupt-file honesty + VIDEO0063 terminal state DONE `6663a4c` (source-res H.264 _rendition_, NOT in-place re-encrypt; self-limiting keyed on `is_source=0`). Only `needs_web_preview` deferred.**
...
9. ~~**E1** (#43)~~ **DONE** `d1f439a` â€” push move, client + E2E, no server change (the move endpoint is direction-agnostic). ~~**E2** (#52) â€” album sort~~ **DONE** `a008500` (web) + `b339a32` (Android) — **and it exposed E3, the viewer's ordering, which is open in `todo.md`.**
10. ~~**B4** (#49) â€” the resolution ladder.~~ **Client-complete and server-complete**: planning/storage/queue (`aee50f8`, `25385bb`, `f3cd439`, `7a12582`, `d1a3079`), generation (`60d555d`, `037`), serving + API (`8564636`), web picker (`163dcc0`), Android picker (`5b46040`), orphan sweeper (`b4548f9`), `blind` count (`037c041`). Largest scope in the batch; it blocked nothing above it.
