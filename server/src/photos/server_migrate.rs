...
/// Encrypt one photo: read from disk → thumbnail → encrypt → write blobs → update DB.
async fn encrypt_one_photo(
    ...
    // Step 2: Generate thumbnail (CPU-bound, offload to blocking thread)
    let thumb_data = {
        let data_clone = file_data.clone();
        let mime_clone = photo.mime_type.clone();
        tokio::task::spawn_blocking(move || generate_thumbnail(&data_clone, &mime_clone))
            .await
            .map_err(|e| format!("Thumbnail task panicked: {}", e))?
    };

    // Step 3: Upload thumbnail blob (if generated)
    ...
    // Step 4: Build the photo payload JSON (same format as the client)
    ...
    // Step 5: Encrypt (CPU-bound, offload to blocking pool)
    let enc_photo = {
        let key_copy = *key;
    ...
    // Step 6: Write encrypted blob to disk
    let blob_id = Uuid::new_v4().to_string();
    let blob_storage_path =
    ...
    // Step 7: Link the plain photo to its encrypted blob (and thumbnail blob)
...
pub async fn start_migration(
    ...
    tokio::spawn(async move {
        run_migration(key, user_id, pool, storage_root, progress).await;
    });
