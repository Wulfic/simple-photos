...
async fn encrypt_one_photo(
    ...
    // Steps 2 & 3: Generate web preview and thumbnail CONCURRENTLY.
    // These are the two most I/O-intensive parts of the pipeline (each may
    // spawn FFmpeg/ImageMagick subprocesses that take seconds for video).
    // Running them in parallel via tokio::join! roughly halves the per-photo
    // wall time for formats that need both operations.
    ...
    let web_preview_fut = async {
        if let Some(preview_ext) = needs_web_preview(&photo.filename) {
            ...
            if tokio::fs::try_exists(&cached_preview_path).await.unwrap_or(false) {
                ...
                tracing::info!("[DIAG:SERVER_MIG] Generating web preview for {} before encryption", photo.filename);
                let success = generate_web_preview_bg(&full_path, &cached_preview_path, preview_ext).await;
                if success {
                    let data = tokio::fs::read(&cached_preview_path).await.unwrap_or_else(|_| file_data.clone());
                    let preview_mime = match preview_ext {
                        "jpg" => "image/jpeg",
                        "png" => "image/png",
                        "mp3" => "audio/mpeg",
