...
/// 1. `{cwd}/downloads/simple-photos.apk` — packaged installs. The service
///    runs with its working dir set to a writable root whose `downloads/`
///    holds the release APK: `/var/lib/simple-photos/downloads` (.deb) and
///    `{app}\downloads` (Windows .exe). This is where the installers' asset
///    fetchers drop the APK pulled from the GitHub release.
/// 2. `{project_root}/android/app/build/outputs/apk/release/app-release.apk`
/// 3. `{project_root}/android/app/build/outputs/apk/debug/app-debug.apk`
/// 4. `{project_root}/downloads/simple-photos.apk`
/// 5. `{static_root}/../downloads/simple-photos.apk` (legacy layouts)
///
...
pub async fn android_apk(State(state): State<AppState>) -> Result<Response, AppError> {
    ...
    let project_root = cwd.join("..");

    let candidates = [
        // Packaged installs (.deb / Windows .exe): the installer asset fetchers
        // write the release APK to `{working-dir}/downloads/`, which the
        // service user can read+write (unlike the read-only package dirs).
        cwd.join("downloads/simple-photos.apk"),
        ...
        // Legacy fallback (kept for older layouts).
        static_root.join("../downloads/simple-photos.apk"),
        static_root.join("../../downloads/simple-photos.apk"),
    ];
