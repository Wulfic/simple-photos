...
///
/// `threads` is the per-encode `-threads` budget, **passed in rather than
/// planned here**. This function used to call
/// `plan_parallelism(num_cpus::get(), ..)` itself, which had two consequences
/// worth stating so it is not re-introduced:
///
/// * it bypassed [`detect_parallelism`], so `SIMPLE_PHOTOS_CONVERSION_JOBS` —
///   the operator's one knob for "keep this box calm" — reached first-pass
///   conversion and **not** the ladder;
/// * it was a *second* derivation of a budget the sweep also needs (it decides
///   how many of these run at once), and two derivations of one number drift.
///   The sweep now plans once and hands both halves down.
pub(crate) async fn transcode_to_rung(
    ...
    if !ok {
        ...
        return Err(format!(
            "ffmpeg failed to produce the {}x{} rung",
            rung.0, rung.1
        ));
    }
    ...
    match tokio::fs::metadata(output).await {
        ...
        Err(e) => Err(format!(
            "rung output missing after a successful encode: {e}"
        )),
    }
}
