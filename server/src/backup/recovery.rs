...
//!
//! **Design notes:**
//! - Recovery deduplicates by **photo ID** — if the local DB already has a
//!   row with the same primary key, the photo is skipped. This is reliable
//!   because synced photos preserve their original UUIDs.
...
async fn run_recovery(
    ...
    // 2. Get local photo IDs for deduplication (more reliable than filename)
    let local_ids: Vec<String> = match sqlx::query_scalar::<_, String>(
    ...
    // 3. Filter to photos not already on this server (by ID)
    let missing: Vec<&BackupPhotoRecord> = remote_photos
        .iter()
        .filter(|p| !local_set.contains(&p.id))
        .collect();
